        ...
        try {
            if (numLongSteps % 2 != 0)
                ...
                /* sphere texture mapping 
					option 1
					source: https://www.mvps.org/directx/articles/spheremap.htm
					u = asin(x)/PI + 0.5 
					v = asin(y)/PI + 0.5 
				*/
				/* option 2
					source https://en.wikipedia.org/wiki/UV_mapping
					[x y z] is unit vector from point of sphere to origin
					u = 0.5 + atan2(z,x)/(2PI)
					v = 0.5 - asin(y)/PI
				*/
				//option 2: 
				var sphereTextureUVs = [0.5 + Math.atan2(0,0)/(2*Math.PI), 0.5 - Math.asin(1)/Math.PI];		
                ...
                for (var latAngle=-latLimitAngle; latAngle<=latLimitAngle; latAngle+=angleIncr) {
                    ...
                    for (var longAngle=0; longAngle<2*Math.PI; longAngle+=angleIncr){ // for each long
						...
						//option 2
						sphereTextureUVs.push(0.5 + Math.atan2(-latRadius*Math.cos(longAngle),-latRadius*Math.sin(longAngle))/(2*Math.PI), 0.5 - Math.asin(-latY)/Math.PI);
						
						}
				} // end for each latitude
                sphereVertices.push(0,1,0); // add north pole
				//option 2
                sphereTextureUVs.push(0.5 + Math.atan2(0,0)/(2*Math.PI), 0.5 - Math.asin(-1)/Math.PI);
...
function initTexture() {
	...
	img.src = "https://ncsucgclass.github.io/prog3/earth.png";
